...
void load_mini_batch(
    ...
    dlib::rand &rnd, const std::vector<std::vector<string>> &objs,
    std::vector<matrix<rgb_pixel>> &images,
    std::vector<unsigned long> &labels) {
  images.clear();
  labels.clear();
  DLIB_CASSERT(num_people <= objs.size(),
               "The dataset doesn't have that many people in it.");
  ...
  for (size_t i = 0; i < num_people; ++i) {
    ...
    for (size_t j = 0; j < samples_per_id; ++j) {
      const auto &obj =
          objs[id][rnd.get_random_32bit_number() % objs[id].size()];
  ...
  // You might want to do some data augmentation at this point.  Here we do some
  ...
  // All the images going into a mini-batch have to be the same size.  And
  // really, all the images in your entire training dataset should be the same
  // size for what we are doing to make the most sense.
  DLIB_CASSERT(images.size() > 0);
  for (auto &&img : images) {
    DLIB_CASSERT(
        img.nr() == images[0].nr() && img.nc() == images[0].nc(),
...
int main(int argc, char **argv) {
  if (argc != 2) {
    cout << "Give a folder as input.  It should contain sub-folders of images "
            "and we will "
         << endl;
    cout << "learn to distinguish between these sub-folders with metric "
            "learning.  "
         << endl;
    cout << "For example, you can run this program on the very small "
            "examples/johns dataset"
         << endl;
  ...
  // Here we do the training.  We keep passing mini-batches to the trainer until
  // the learning rate has dropped low enough.
  ...
  // Now, just to show an example of how you would use the network, let's check
  // how well it performs on the training data.
  dlib::rand rnd(time(0));
  load_mini_batch(5, 5, rnd, objs, images, labels);

  // Normally you would use the non-batch-normalized version of the network to
  // do testing, which is what we do here.
  ...
  // Now, check if the embedding puts images with the same labels near each
  // other and images with different labels far apart.
  int num_right = 0;
  int num_wrong = 0;
  for (size_t i = 0; i < embedded.size(); ++i) {
    for (size_t j = i + 1; j < embedded.size(); ++j) {
      if (labels[i] == labels[j]) {
        // The loss_metric layer will cause images with the same label to be
        // less than net.loss_details().get_distance_threshold() distance from
        // each other.  So we can use that distance value as our testing
        // threshold.
        if (length(embedded[i] - embedded[j]) <
            testing_net.loss_details().get_distance_threshold())
          ++num_right;
        else
          ++num_wrong;
      } else {
